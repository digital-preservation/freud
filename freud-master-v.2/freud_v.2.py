...
# Determine the directory where the script lives
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# below loads the csv file into pandas and takes required columns
...
columns_needed = [
    ...
    'SHA256_HASH', 'FORMAT_COUNT', 'PUID', 'MIME_TYPE',
...
# Load supporting CSVs from the script directory
originalWhiteList = pd.read_csv(os.path.join(BASE_DIR, 'approved_formats.csv'))
originalFlaggedFormats = pd.read_csv(os.path.join(BASE_DIR, 'Flagged_Formats.csv'))
originalFurtherResearch = pd.read_csv(os.path.join(BASE_DIR, 'Further_Research.csv'))
originalNotInDRI = pd.read_csv(os.path.join(BASE_DIR, 'DRI_allowed_formats.csv'))
...
def NotInAllowedFormats():

    # Filter the main CSV
    df = csv.loc[(csv['FORMAT_COUNT'] > 0) & (csv['SIZE'] > 0), :]

    # Load allowed formats
    allowed = originalNotInDRI["PUID"].dropna().unique().tolist()

    # Select formats NOT in allowed list
    not_allowed = df.loc[~df["PUID"].isin(allowed)]
    not_allowed = not_allowed.sort_values("PUID")

    # Excel formatting
    workbook = results.book
    ...
    not_allowed.to_excel(
        results,
        sheet_name='Not_Allowed_Formats',
        index=False,
        startcol=0,
        startrow=1
    )
    ...
    # Access sheet
    sheet = results.sheets['Not_In_DRI_Format_List']

    # Build a long header row (same style as your FurtherResearch sheet)
    header_text = [
        'FORMATS NOT IN DRI SYSTEM', '', '', '',
        'These file formats appear in this collection but will require an update to DRI to ingest. '
        'Please let a digital archivist know before transfer.',
        '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', ''
    ]

    # Write header across A1:X1
    sheet.write_row('A1:X1', header_text, header_format)

    sheet.write_row(0, 0, header_text, header_format)
...
print("Saving results to:", os.path.abspath(droidname + '_freudresults.xlsx'))